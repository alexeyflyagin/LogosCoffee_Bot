...
class BTN(StrGroup):
    ...
    WRITE_PROMOTIONAL_OFFER = "📢 Новая акция"
    REVIEWS = "💬 Отзывы"
...
class GENERAL(StrGroup):
    ...
    class LOGIN(StrGroup):
        KEY_WAS_NOT_ENTERED = "Для входа нужен ключ! Просто напишите его после /start."

        INVALID_KEY = "Упс! Ключ неверный. Попробуте снова 🔑"
...
class ADMIN(StrGroup):
    ...
    class MAKE_OFFER(StrGroup):
        ENTER_CONTENT = dedent(f"""\
            ✍️ Отправьте контент акционного предложения. 
            Это должен быть текст. Можете прикрепить одно фото 📸 для превью.
            {GENERAL.CANCEL_ACTION}
        """)
    ...
    class OFFER(StrGroup):
        ...
        MAIN = dedent(f"""\
            {b('Акция')} {code(f'#{{offer_id}}')}
            Создана: {{date_create}}
            Последняя рассылка: {{date_last_distribute}}
        """)

        DOES_NOT_EXIST = "Акция не найдена."

        class PUBLISH(StrGroup):
            SUCCESSFUL = "Акция успешно опубликована!"

            WARNING = dedent(f"""\
                ⚠️ {b(f'Внимание: Проверьте корректность информации в акции!')} {i(f'({BTN.CANCEL} » {BTN.SHOW})')}
                После подтверждения, всем клиентам будет разослано данное предложение.
                
                Вы уверены, что хотите сделать рассылку {b('акции #{offer_id}')}?
            """)

            COOLDOWN_ERROR = """Недавно вы уже делали рассылку. Пожалуйста, попробуйте чуть позже ⏳"""

        class DELETE(StrGroup):
            SUCCESSFUL = "Акция успешно удалена!"

            WARNING = dedent(f"""\
                ⚠️ {b('Внимание!')} После подтверждения, акция будет удалена из архива.
                Вы уверены, что хотите удалить {b('акцию #{offer_id}')}?
            """)
