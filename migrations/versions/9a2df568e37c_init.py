...
def downgrade() -> None:
    ...
    op.execute(text('DROP TRIGGER IF EXISTS trigger_new_order ON "client_order";'))
    op.execute(text("""DROP FUNCTION notify_new_order()"""))

    op.execute(text('DROP TRIGGER IF EXISTS trigger_update_announcement ON "announcement";'))
    op.execute(text("""DROP FUNCTION notify_new_distributed_announcement()"""))

    op.execute(text('DROP TRIGGER IF EXISTS trigger_new_review ON "review";'))
